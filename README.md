...
### 分表分区（按日分表/按月分表）
一般不按照数据量计算，oracle给的建议是2G，数据量能大于2G，那么就需要分区。具体的数据量没给那么清晰。
...
### 是否有最大查询时间跨度限制
首先要根据涉及表存储数据量进行预先判断：  
* 如果表未采用分表且数据量随着时间的累积远小于100万，则没有查询时间跨度限制；
* 如果表未采用分表且数据量随着时间的累积会超过100万，则没有查询时间跨度限制；
