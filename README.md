...
## 一、表结构设计  
### 数据采集频率(准实时、少于N次/天)  
目前，营业数据库a/b/c/d中心连接数采集频率为10分钟一次，每天采集6*24=144次；     
目前，营业数据库session连接采集频率为30分钟一次，每天采集2*24=48次；   
因此，数据采集频率太快了也不行，高频度采集会导致表每日增量剧增，以后势必导致查询慢，规定查询频率不得高于10min/1次；
考虑到每个定时采集任务在同一时间点不能无限多个，有一定的线程数量限制，规定不能超过20个；  
### 采集一次数据量  
目前，营业数据库a/b/c/d中心连接数采集一次数据量为171条；    
目前，营业数据库session连接数采集一次数据量为25000条左右；   
因此，建议采集一次数据量也不要太大，数据量过多导致表每日增量剧增，以后势必导致查询慢；
### 表理论每日增量  
目前，营业数据库a/b/c/d中心连接数表理论每日增量为171*144=24624条；  
目前，营业数据库session连接数表理论每日增量为25000*48=120万条，尽管现在arch_session_yyyymmdd已经建立日表，但是每天百万以上的数据量查询依旧性能很差；  
表理论增量=数据采集频率*采集一次数据量；可以根据表理论每日增量进行分表策略，规定表理论每日增量不得超过100w条；    

### 单表最大数据量级  
oracle推荐是单表500万，考虑到业务上面大都是连表查询，索引利用率，查询字段多等，这里本项目规定单表最大数据量级为100万；
### 数据保留时间  
...
### 建表方式(新建、复用单表或分表分区等) 
根据表的以后存储的数据数量级判定，当一个应用的数据量大的时候，我们用单表和单库来存储会严重影响操作速度，如oracle存储，我们经过测试，100w以下的时候，oracle的访问速度都还比较快，但是如果超过100w以上的数据，他的访问速度会急剧下降，影响到我们webapp的访问速度，而且数据量太大的话，如果用单表存储，就会使得系统相当的不稳定，oracle服务很容易挂掉。所以当数据量超过100w的时候，建议还是考虑分表.   
以下是平台常见的分表算法。    
#### 按自然时间来分表/分库;   
...
* 此外，如果数据量在一天内的数据量仍很多，可以继续按照业务等拆分等；  
### 建索引  
...
### 缓存（在持久层或持久层之上做缓存）  
日后可以考虑引入到我们项目中， [redis官网](http://redis.io/download/ "Title") 下载Java redis驱动，通过jedis-2.9.0.jar去连接redis服务，进而进行增删查改操作，优势如下：  
1，Nosql数据库，数据存储在缓存中读取速度性能极高，支持分布式；  
2，redis可以定时把内存数据同步到磁盘，将数据持久化；  
...
* 如果一个应用的数据在一年后数据量会达到100w左右，那么我们就设定最大查询时间跨度限制为1年；  
* 如果数据量在一个月就达到了100w左右，那么我们就设定最大查询时间跨度限制为1个月；    
* 如果数据量在某些天就达到100w左右，就设定最大查询时间跨度限制为几天；    
