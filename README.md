...
## 一、表结构设计  
### 数据采集频率(准实时、少于N次/天)  
目前，营业数据库a/b/c/d中心连接数采集频率为10分钟一次，每天采集6x24=144次；     
目前，营业数据库session连接采集频率为30分钟一次，每天采集2x24=48次；   
...
### 表理论每日增量  
目前，营业数据库a/b/c/d中心连接数表理论每日增量为171x144=24624条；  
目前，营业数据库session连接数表理论每日增量为25000x48=120万条，尽管现在arch_session_yyyymmdd已经建立日表，但是每天百万以上的数据量查询依旧性能很差；  
表理论增量=数据采集频率x采集一次数据量；可以根据表理论每日增量进行分表策略，规定表理论每日增量不得超过100w条；    
